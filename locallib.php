...
/** Custom functions to filter-out users in grade page for users other than admin **/

/**
 * Filter users based on matching batchcodes.
 *
 * @param array $users List of user IDs.
 * @return array Filtered list of user IDs.
 */
function filter_batchcodes_records($users) {
    global $USER, $DB;

    $has_role = get_participant_role();
    if (is_siteadmin() || !$has_role) {
        return $users;
    }
    ...
    // Get trainer's batchcodes (handle multiple).
    $batchcodestring = $USER->profile['batchcode'] ?? '';
    $trainerbatchcodes = array_map('trim', explode(',', $batchcodestring));

    // Batch load user records.
    list($sql, $params) = $DB->get_in_or_equal(array_values($users));
    $userrecords = $DB->get_records_select('user', "id $sql", $params);

    foreach ($userrecords as $user) {
        profile_load_custom_fields($user);
        $userbatchcode = $user->profile['batchcode'] ?? '';

        if (empty($userbatchcode) || !in_array($userbatchcode, $trainerbatchcodes)) {
            unset($users[array_search($user->id, $users)]);
        }
    }
...
/**
 * Filter participants for dropdowns based on matching batchcodes.
 *
 * @param array $participants List of participant objects (id => user object).
 * @return array Filtered list of participant objects.
 */
function filter_batch_code_based_user_dropdown($participants) {
    global $USER;
    ...
    foreach ($participants as $participant) {
        profile_load_custom_fields($participant);
        $userbatchcode = $participant->profile['batchcode'] ?? '';
...
/**
 * Check if current user has the 'trainer' role.
 *
 * @return bool True if user is trainer, false otherwise.
 */
function get_participant_role() {
    global $USER, $DB;

    static $has_role = null; // Cache result to avoid repeat DB calls.

    if ($has_role !== null) {
        return $has_role;
    }

    $custom_role = $DB->get_record('role', array('shortname' => 'trainer_new'), 'id');
    if (!$custom_role) {
        return false;
    }

    $has_role = user_has_role_assignment($USER->id, $custom_role->id);
